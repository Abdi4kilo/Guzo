...
Meteor.methods({
	...
	var hitchinfo={hitcherId:Meteor.userId(),hitchername: Meteor.users.findOne({_id:Meteor.userId()}).profile.full_name,departuer:departuer,destination:destination,status:"undetermined"};	
	var driverId=Routes.findOne({_id:rideId}).createBy;
	 var driverName=Meteor.users.findOne({_id:driverId}).profile.full_name;
	 Hitcher.insert({hitcherId:Meteor.userId(),drivername:driverName,hitchername: Meteor.users.findOne({_id:Meteor.userId()}).profile.full_name,departuer:departuer,destination:destination,status:"undetermined"});	
